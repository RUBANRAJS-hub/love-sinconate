...
import { Sparkles, Gamepad2, Heart, RefreshCw } from 'lucide-react';
import confetti from 'canvas-confetti';

export const MiniGames: React.FC = () => {
  const [activeGame, setActiveGame] = useState<'scratch' | 'puzzle'>('scratch');
    ...
    <div className="relative min-h-screen w-full flex flex-col items-center pt-24 pb-20 px-6 bg-[#0D1117]">
      ...
      <div className="text-center max-w-lg mb-10 z-10 select-none">
        ...
        <div className="inline-flex mt-6 bg-white/5 border border-white/5 p-1 rounded-full backdrop-blur-md">
          <button
            ...
            className={`px-5 py-1.5 rounded-full text-xs font-semibold tracking-wide transition-all cursor-pointer ${
      ...
      <div className="w-full max-w-lg z-10 flex flex-col items-center justify-center">
        <AnimatePresence mode="wait">
          {activeGame === 'scratch' ? (
            <ScratchCardGame key="scratch-card" />
          ) : (
            <HeartPuzzleGame key="heart-puzzle" />
          )}
        </AnimatePresence>
      </div>
    </div>
